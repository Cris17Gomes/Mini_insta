...
const novaPostagem = async (req, res) => {
  ...
  try {
    ...
    if (!fotosCadastradas) {
      await knex("postagens").where({ id: postagem[0].id }).delete();
      return res.status(400).json("Não foi possível cadastrar a postagem");
    }

    return res.status(200).json("Postagem cadastrada com sucesso");
  } catch (error) {
    console.log({ mensagem: "é aqui" });
...
const curtir = async (req, res) => {
  ...
  try {
    ...
    if (jaCurtiu) {
      return res.status(404).json("Você já curtiu essa postagem");
    }

    const curtida = await knex("postagem_curtidas")
      .insert({ usuario_id: id, postagem_id: postagem.id })
      .returning("*");
...
const comentar = async (req, res) => {
  ...
  try {
    ...
    const comentario = await knex("postagem_comentarios")
      .insert({ usuario_id: id, postagem_id: postagem.id, texto })
      .returning("*");

    if (!comentario) {
      return res.status(400).json("Não foi possível comentar a postagem");
    }
